...
/**
 * EmailTemplateService — Génère le contenu HTML et texte des emails.
 *
 * SRP: Ce service NE fait QUE construire le contenu des emails (templates).
 *      Il n'envoie PAS (→ EmailTransportService).
 *      Il ne connaît PAS la logique métier (→ EmailService).
 *
 * Types d'emails supportés:
 *   - buildCertificateAlert: Alerte de changement de statut d'alarme
 *   - buildOtpVerification: Code OTP pour vérification d'adresse email
 *   - buildUserInvitation: Invitation + mot de passe temporaire utilisateur
 */
@Injectable()
export class EmailTemplateService {
  /** Template d'alerte de changement d'alarme sur un certificat */
  buildCertificateAlert(
    vessel: any,
    cert: any,
    prevAlarm: string,
  ): { subject: string; text: string; html: string } {
    const alarmLevel: string = cert.alarm_status;
  ...
  /** Template OTP pour vérification d'adresse email de navire */
  ...
  /** Template d'invitation pour un nouveau compte utilisateur */
  buildUserInvitation(
    name: string,
    otp: string,
  ): { subject: string; text: string; html: string } {
