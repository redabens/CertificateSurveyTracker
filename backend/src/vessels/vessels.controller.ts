import { Controller, Get, Post, Delete, Put, Param, Body, UseGuards, Req, Res, UseInterceptors, UploadedFile, ForbiddenException, BadRequestException } from '@nestjs/common';
...
export class VesselsController {
  ...
  async getAll(@Req() req: any) {
    ...
    return vessels.map(v => {
      const certs = this.vesselsService['db'].prepare('SELECT * FROM certificates WHERE vessel_id = ?').all(v.id) as any[];
  ...
  async importExcel(@Req() req: any, @UploadedFile() file: any) {
    ...
    try {
      ...
      if (existing) {
        ...
        throw new BadRequestException(`Le navire "${vInfo.name}" existe déjà dans le système`);
      }
      ...
      if (file && fs.existsSync(file.path)) fs.unlinkSync(file.path);
      throw new BadRequestException('Erreur lors du traitement du fichier Excel: ' + err.message);
    }
  }
  ...
  async getSettings(@Param('id') vesselId: string) {
    let settings = this.vesselsService['db'].prepare('SELECT * FROM email_settings WHERE vessel_id = ?').get(parseInt(vesselId)) as any;
    if (!settings) {
  ...
  @Put(':id/settings')
  async updateSettings(@Req() req: any, @Param('id') vesselId: string, @Body() body: any) {
    if (req.user.role !== 'Admin') {
      throw new ForbiddenException('Action interdite pour votre profil');
    }
