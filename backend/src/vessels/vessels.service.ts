  ...
  async update(id: number, v: any): Promise<any> {
    ...
    const updated = await this.prisma.vessel.update({
      ...
      data: {
        ...
        grossTonnage:
          v.gross_tonnage !== undefined
            ? v.gross_tonnage
              ? parseInt(v.gross_tonnage, 10)
              : 0
            : existing.grossTonnage,
        deadweightTonnage:
          v.deadweight_tonnage !== undefined
            ? v.deadweight_tonnage
              ? parseInt(v.deadweight_tonnage, 10)
              : 0
            : existing.deadweightTonnage,
        portOfRegistry:
          v.port_of_registry !== undefined
            ? v.port_of_registry || null
            : existing.portOfRegistry,
        callSign:
          v.call_sign !== undefined ? v.call_sign || null : existing.callSign,
        yearBuilt:
          v.year_built !== undefined
            ? v.year_built
              ? parseInt(v.year_built, 10)
              : null
            : existing.yearBuilt,
        classSociety:
          v.class_society !== undefined
            ? v.class_society || null
