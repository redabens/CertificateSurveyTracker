    ...
    async generateExcelExport(vesselId, lang) {
        ...
        const formattedCertificates = certificates.map((cert) => {
            ...
            if (windowText && windowText.trim().startsWith('[')) {
                try {
                    const arr = JSON.parse(windowText);
                    windowText = arr
                        .map((item) => {
                        if (item.mode === 'custom') {
                    ...
                    console.warn('[VesselsService] Failed to parse structured window text in Excel export:', e);
                }
            }
...
function formatDateStringExcel(dateStr) {
    if (!dateStr)
        return '';
